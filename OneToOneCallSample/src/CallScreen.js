...
export default class CallScreen extends Component {
  ...
  componentDidMount() {
    ...
    if (isOutgoingCall) {
      ...
      this.setState({isShowDeclineBt:false, isShowEndBt:true, isShowAcceptBt:false, isShowOptionView:true , isOutgoingCall:isOutgoingCall, userId:to, isVideoCall:isVideoCall});
      ...
      this.setState({isShowDeclineBt:true, isShowEndBt:false, isShowAcceptBt:true, isShowOptionView:false, isOutgoingCall:isOutgoingCall, userId:from, callId:callId, isVideoCall:isVideoCall});
  ...
  _callDidChangeMediaState = ({callId, code, description}) => {
    ...
    switch (code) {
      case 0:
      // Connected
      this.setState({callState:description});
      break;
      case 1:
      // Disconnected
      break;
  ...
  _onMutePress = () => {
    this.refs.stringeeCall.mute(this.state.callId, !this.state.isMute, (status, code, message) => {
  ...
  _onSpeakerPress = () => {
    this.refs.stringeeCall.setSpeakerphoneOn(this.state.callId, !this.state.isSpeaker, (status, code, message) => {
      if (status) {
  ...
  _onSwitchCameraPress = () => {
    this.refs.stringeeCall.switchCamera(this.state.callId, (status, code, message) => {
  ...
  _onVideoPress = () => {
    if (this.state.isVideoCall) {
      this.refs.stringeeCall.enableVideo(this.state.callId, !this.state.isEnableVideo, (status, code, message) => {
        if (status) {
  ...
  render () {
    return (
      <View style={styles.container}>
        ...
        {this.state.hasReceivedLocalStream && this.state.callId != '' && <StringeeLocalVideoView 
        style={styles.remoteView} 
        callId={this.state.callId} 
        streamId='' 
        />}

        {this.state.isVideoCall && <TouchableOpacity
        onPress={this._onSwitchCameraPress} style = {styles.camera}>
          <Image source={require('../resource/camera_switch.png')} style = {{width:40, height:40}} />
        </TouchableOpacity>}
